...
func DeploySimpleMultiSigContract(nonceBucketNum uint16, chainID *big.Int, backend bind.ContractBackend, privkHex string, hexAddress []string, mRequired uint8) (string, error) {
	...
	add, tx, _, err := sms.DeploySimpleMultiSig(auth, backend, nonceBucketNum, big.NewInt(int64(mRequired)), owners, chainID)
	if err != nil {
...
func GetContractInfo(caller bind.ContractCaller, contractAddress string) (addrs []string, mRequired int64, err error) {
	var multisigContract *sms.SimpleMultiSigCaller

	{ // init vars
		multisigContract, err = sms.NewSimpleMultiSigCaller(common.HexToAddress(contractAddress), caller)
		if err != nil {
...
func ExecuteTX(txp *TxParams) (string, error) {
	var (
		err              error
		privk            *ecdsa.PrivateKey
		multisigContract *sms.SimpleMultiSig
	)
	...
	{ // init vars
		...
		multisigContract, err = sms.NewSimpleMultiSig(common.HexToAddress(txp.MultisigContractAddress), txp.Backend)
		if err != nil {
